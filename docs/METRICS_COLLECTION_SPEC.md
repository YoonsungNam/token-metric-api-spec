...
> 설계 배경·근거: [의사결정 로그](internal/DECISIONS.md) · [미팅용 요약](METRICS_COLLECTION_SUMMARY.md)
...
| **모든 서비스** | [2] GET `/v1/metrics` 구현 — gpu 블록 (§2~3) | **~9/9** |
| **GPU 서빙 팀 전체** (케이스 A~D·F) | [3a] serving 블록 자체 집계 (§4 — A~C는 엔진 히스토그램 활용 가능) | ~9/9 |
...
> ⚠️ **일정 변경**: **9/14 대표이사 보고**를 위해 구축 기한이 기존 ~9/18 → **~9/9**로 앞당겨졌다.
...
- 미등록 모델 표기가 데이터에 나타나면 운영자가 후보와 함께 알림 → **"기존 ○○의 alias" / "새 모델" 중 택일 회신**하면 끝 (사전 갱신 후 14일 내 소급 재처리. 날짜 접미 패턴은 자동 처리, 무응답 시 "미등록"으로 계속 표시)
...
>
> serving 배열에는 **gpu 블록에 serving 행이 있는 모델의 성능 행**을 넣는다 (standby·test만 있는 모델은 의무 없음). 사내 플랫폼에서 소비하는 모델의 행은 넣지 않고(성능은 플랫폼이 집계·제공), **자체 GPU 없이 플랫폼만 소비하는 서비스는 `gpu: []`·`serving: []`** 로 응답한다. 사외 API 직접 호출 모델의 행은 선택(케이스 E 준용). 그날 **요청이 0건**이었던 모델의 성능 행은 생략할 수 있다. gpu 블록은 케이스와 무관하게 **자체 GPU 사용분 전부**를 담는다 (§3).
...
- 최근 **14일 backfill** 가능 — 정정 시 담당자가 재집계 후 운영자에게 재수집 요청. (연휴·휴가로 조치가 늦어져도 정정 창을 놓치지 않도록 2주로 설정)
